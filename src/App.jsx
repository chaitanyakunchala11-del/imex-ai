...
const S = {
  card:{
    background:"linear-gradient(160deg,#111009 0%,#0d0c0a 100%)",
    border:"1px solid rgba(200,169,110,0.18)",
    borderRadius:12,
    padding:"28px 24px",
    marginBottom:16,
    boxShadow:"0 4px 32px rgba(0,0,0,0.4), inset 0 1px 0 rgba(200,169,110,0.06)",
  },
  lbl:{
    fontSize:9,
    color:"#c8a96e",
    ...
    <div style={{ minHeight:"100vh", background:"#0e0c0a", fontFamily:"'DM Mono',monospace", color:"#e8e0d0", paddingBottom:80 }}>
      ...
      <div style={{ maxWidth:740, margin:"0 auto", padding:"20px 14px 0" }}>
        ...
        <div style={S.card}>
          ...
          {/* Section heading */}
          <div style={S.lbl}>
            <span style={{ width:18, height:1, background:"linear-gradient(90deg,#c8a96e,rgba(200,169,110,0.3))", display:"inline-block" }}/>
            Shipment Details
          </div>
          ...
          <div className="imex-g2" style={{ marginBottom:18 }}>
            <F label="Origin Country">
              <select style={S.sel} value={form.origin} onChange={e=>set("origin",e.target.value)}
                ...
                onBlur={e=>{e.target.style.borderColor="rgba(200,169,110,0.2)";e.target.style.boxShadow="none";}}>
                {Object.entries(COUNTRIES).map(([k,v])=><option key={k} value={k}>{v}</option>)}
              </select>
            </F>
            <F label="Destination Country">
            ...
            {(() => {
              ...
              return (
                <div style={{
                  marginTop:10,
                  padding:"14px 16px",
                  background:"rgba(78,200,138,0.04)",
                  border:"1px solid rgba(78,200,138,0.2)",
                  borderRadius:8,
                  boxShadow:"inset 0 1px 0 rgba(78,200,138,0.06)",
                }}>
                  <div style={{ fontSize:9, color:"#4ec88a", letterSpacing:"0.2em", textTransform:"uppercase", marginBottom:6 }}>
                    ◆ Suggested — {hint.label}
                  </div>
                  <div style={{ fontSize:12, color:"#8a8070", lineHeight:1.7, marginBottom:isBioCat||form.category!==hint.category?8:0 }}>
                    HS code: <span style={{ color:"#c8a96e", fontFamily:"monospace", fontWeight:600, fontSize:13 }}>{hint.hs}</span>
                    <span style={{ color:"#5a5040" }}> — verify with ABF Tariff Finder or a licensed customs broker.</span>
                  </div>
                  {isBioCat && (
                    <div style={{ fontSize:11, color:"#c8903a", lineHeight:1.65, marginBottom:form.category!==hint.category?8:0 }}>
                      ⚑ Biosecurity / food compliance may apply. Check DAFF BICON before ordering.
                    </div>
                  )}
                  {form.category !== hint.category && (
                    <button type="button" onClick={()=>set("category",hint.category)} style={{
                      ...
                      border:"1px solid rgba(78,200,138,0.3)",
                      borderRadius:5,
                      padding:"5px 12px",
                      color:"#4ec88a",
                      fontFamily:"'DM Mono',monospace",
          ...
          <div className="imex-g3" style={{ marginBottom:18 }}>
            <F label="Quantity (units)">
              <input style={S.inp} type="number" min="1" placeholder="e.g. 200" value={form.qty} onChange={e=>set("qty",e.target.value)}
                onFocus={e=>{e.target.style.borderColor="rgba(200,169,110,0.55)";e.target.style.boxShadow="0 0 0 3px rgba(200,169,110,0.08)";}}
                onBlur={e=>{e.target.style.borderColor="rgba(200,169,110,0.2)";e.target.style.boxShadow="none";}}
          ...
          <div style={{ marginBottom:18 }}>
            ...
            {form.incoterm && (
              <div style={{ fontSize:11, color:"#5a5040", marginTop:8, lineHeight:1.65, paddingLeft:2 }}>{incotermDesc}</div>
            )}
          </div>
          ...
          {/* Biosecurity + Display Currency */}
          <div className="imex-g2" style={{ marginBottom:18 }}>
            <F label={`Biosecurity / Inspection Fee (USD)${BIO_CATEGORIES.includes(form.category) ? " — may apply" : ""}`}>
              <input style={S.inp} type="number" min="0"
                placeholder={BIO_CATEGORIES.includes(form.category) ? "e.g. 180" : "0"}
                value={form.bioFee} onChange={e=>set("bioFee",e.target.value)}
                onFocus={e=>{e.target.style.borderColor="rgba(200,169,110,0.55)";e.target.style.boxShadow="0 0 0 3px rgba(200,169,110,0.08)";}}
          ...
          {/* Packaging subsection — bordered inner box */}
          <div style={{
            marginTop:4,
            ...
            <div style={{ fontSize:9, color:"#7a6040", letterSpacing:"0.26em", textTransform:"uppercase", marginBottom:14, display:"flex", alignItems:"center", gap:8 }}>
              <span style={{ width:12, height:1, background:"rgba(200,169,110,0.4)", display:"inline-block" }}/>
              Retail Packaging Costs per Pack — AUD, optional
            </div>
            <div className="imex-g2" style={{ gap:12, marginBottom:10 }}>
              <F label="100g pack cost">
                <input style={S.inp} type="number" min="0" step="0.01" placeholder="0.20" value={form.pkg100} onChange={e=>set("pkg100",e.target.value)}
                  onFocus={e=>{e.target.style.borderColor="rgba(200,169,110,0.55)";e.target.style.boxShadow="0 0 0 3px rgba(200,169,110,0.08)";}}
                  onBlur={e=>{e.target.style.borderColor="rgba(200,169,110,0.2)";e.target.style.boxShadow="none";}}
            ...
            <div style={{ fontSize:10, color:"#3a3020", lineHeight:1.6 }}>
              Defaults: 100g = A$0.20 · 250g = A$0.25 · 500g = A$0.35 · 1kg = A$0.50. Enter your real packaging costs for accurate retail prices.
            </div>
          </div>
